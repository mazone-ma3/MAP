/* MSX2 キャラMAP移動実験  2x2圧縮展開 By m@3 */
/* DISK BASIC only. */
/* キャラを出す */
...
void chr_sub(void)
{
	int no2 = chr_tbl[dir * 2 + dir2][(i - CHR_X) + (j - CHR_Y) * 2];

	VDPsetAREA((no & 0x0f) * PARTS_X, (no & 0xf0) / (PARTS_Y / 4) + 256, 0, 212, PARTS_X, PARTS_Y, 0,0, HMMM);

	VDPsetAREA((no2 & 0x0f) * PARTS_X + (128 * (PARTS_X / 8)), (no2 & 0xf0) / 2 + 256, 0, 212, PARTS_X, PARTS_Y, 0,0, LMMM | 0x08);
...
void pat_sub(void)
{
	VDPsetAREA((no & 0x0f) * PARTS_X, (no & 0xf0) / (PARTS_Y / 4) + 256, vx, vy, PARTS_X, PARTS_Y, 0,0, HMMM);
}
...
//					jj = j * 2 + yy;
					...
					*map_adr = *(pat_adr);
					map_adr -= (32 - 1);
/*
					map_data[ii + jj * 32] = *(pat_adr++);
					map_data[(ii + 1) + jj * 32] = *(pat_adr++);
					map_data[ii + (jj + 1) * 32] = *(pat_adr++);
					map_data[(ii + 1) + (jj + 1) * 32] = *(pat_adr);
*/
					++data;
				}
				data_tmp += MAP_SIZE_X;
