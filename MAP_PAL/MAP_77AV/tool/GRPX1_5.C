...
/* 2分割する 拡大しない 右半分版  マスク抽出(1プレーン分のみ) */
...
unsigned char conv_tbl[16] = { 0, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 ,7 ,7 ,7 ,7 };
...
int conv(char *loadfil, char*savefil1, char *savefil2)
{
	...
	for(count = 0; count < LINE * 2; ++count){
		...
		for(count2 = 0; count2 < WIDTH / 2 / 2 / 2; ++count2){
...
//			msxcolor[1] = (pattern[0] >>4) & 0x0f;
			msxcolor[1] = pattern[0] & 0x0f;
//			msxcolor[3] = pattern[0] & 0x0f;
			msxcolor[2] = (pattern[1] >>4) & 0x0f;
//			msxcolor[5] = (pattern[1] >>4) & 0x0f;
			msxcolor[3] = pattern[1] & 0x0f;
//			msxcolor[7] = pattern[1] & 0x0f;
			msxcolor[4] = (pattern[2] >>4) & 0x0f;
			msxcolor[5] = pattern[2] & 0x0f;
			msxcolor[6] = (pattern[3] >>4) & 0x0f;
			msxcolor[7] = pattern[3] & 0x0f;
			for(i = 0; i < 3; ++i){
				x1color[i] = 0;
			}
...
//			}
			...
			if(size < 0x2000){ //WIDTH * LINE * 3
				i = fwrite(pattern, 1, 2, stream[1]);	/* 8dot分*2 */
				size += i;
				if(i < 1)
					break;
			}else{
				i = fwrite(pattern, 1, 2, stream[2]);	/* 8dot分*2 */
				size += i;
				if(i < 1)
					break;
			}
