...
//	_FP_OFF(vram) = (x / 2) + (y) * 512;
	...
	for (j = 0; j < 16; j++){
		for(i = 0; i < 4; i++){
//			chr_8[i][j][no] = *(vram++);
//			chr_8[i][j][no] = _peek_word (0x120, vram);
			chr_8[i][j][no] = VRAM_getPixelW(vram);
			vram += 2;
		}
		vram += (256 - 4) * 2;
	}
...
//	_FP_OFF(vram) = (x / 2) + (y) * 512;
	...
	for (j = 0; j < 16; j++){
		for(i = 0; i < 4; i++){
//			chr_8_2[i][j][no] = *(vram++);
//			chr_8_2[i][j][no] = _peek_word (0x120, vram);
			chr_8_2[i][j][no] = VRAM_getPixelW(vram);
			vram += 2;
		}
		vram += (256 - 4) * 2;
	}
...
//	_FP_OFF(vram) = (x / 2) + (y) * 512;
	...
	for (j = 0; j < 16; j++){
		for(i = 0; i < 4; i++){
//			*(vram++) = chr_8[i][j][no];
//			_poke_word(0x120, vram, chr_8[i][j][no]);
			VRAM_putPixelW( vram, chr_8[i][j][no]);
			vram += 2;
		}
		vram += (256 - 4) * 2;
	}
...
//			_poke_word(0x120, vram, chr_8_2[i][j][no2]);
			VRAM_putPixelW( vram, chr_8_2[i][j][no2]);
			vram += 2;
		}
		vram += (256 - 4) * 2;
	}
...
//			*vram_adr = color;
			_poke_byte(0x120, (j + i * 512) * 1, color);
		}
	}
}
...
//			palram[color] = green * 64 * 32 + red * 64 + blue * 2;
		...
		case 0:
			...
			outp(0x448,0x01);
			outp(0x44a,0x21);	/* priority register */
